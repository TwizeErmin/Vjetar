...
keyauthapp = api(
    name = "crkva", #App name (Manage Applications --> Application name)
    ownerid = "pTvtz6Ck6v", #Owner ID (Account-Settings --> OwnerID)
    secret = "c888bcf35ff19e792c49adc3a98d837d8901e0ac02630aa5f9b071cbbac92cfb", #App secret(Manage Applications --> App credentials code)
    version = "1.0",
...
import pydirectinput, random, win32api, win32con, sys, threading
...
GreenBarColor = 60, 184, 121
GreenBarColor1 = 60, 161, 60
...
while not event.is_set():
    pyautogui.press('e')
    kod = pyautogui.locateCenterOnScreen('kod.png', confidence=0.7)
    riba1 = pyautogui.locateCenterOnScreen('riba1.png', grayscale=True, confidence=0.7)
    riba2 = pyautogui.locateCenterOnScreen('riba2.png', grayscale=True, confidence=0.7)
    riba3 = pyautogui.locateCenterOnScreen('riba3.png', grayscale=True, confidence=0.7)
    odbij = pyautogui.locateCenterOnScreen('odbij.png', confidence=0.7)
    ...
    if upecao > 90:
        pyautogui.press('i')
        time.sleep(1)
        hrana = pyautogui.locateCenterOnScreen('voda.png', confidence=0.7)
        time.sleep(4)
        pyautogui.moveTo(hrana)
        pyautogui.click(clicks=1, interval=1)
        time.sleep(0.5)
        use = pyautogui.locateCenterOnScreen('use.png', confidence=0.7)
        time.sleep(4)
        pyautogui.moveTo(use)
        pyautogui.click(clicks=1, interval=1)
        pyautogui.press('i')
        time.sleep(1)
        voda = pyautogui.locateCenterOnScreen('hrana.png', confidence=0.8)
        time.sleep(4)
        pyautogui.moveTo(voda)
        pyautogui.click(clicks=1, interval=1)
        time.sleep(0.5)
        use = pyautogui.locateCenterOnScreen('use.png', confidence=0.7)
        time.sleep(4)
    ...
    if kod:
        img = ImageGrab.grab(bbox=(590, 170, 1030, 270))  # x, y, w, h
        img_np = np.array(img)
        frame1 = cv2.cvtColor(img_np, cv2.COLOR_BGR2GRAY)
        thresh = 128
        frame = cv2.threshold(frame1, thresh, 255, cv2.THRESH_BINARY)[1]
        filename = 'Text_Captured.jpg'
        cv2.imwrite(filename, frame)
        t5 = pytesseract.image_to_string(Image.open(filename), lang="eng")
        ...
        for i in t5:
            if not " " in i or not "" in i:
                t6 = str(t6)
                t6 = t6 + i
        t6 = t6.translate({ord("'"): None})
        t6 = t6.translate({ord("‘"): None})
        t6 = t6.translate({ord("`"): None})
        t6 = t6.translate({ord("_"): None})
        t6 = t6.translate({ord("-"): None})
        t6 = t6.translate({ord("+"): None})
        t6 = t6.translate({ord(";"): None})
        t6 = t6.translate({ord(":"): None})
        ...
        if pyautogui.locateOnScreen(f'kod.png', grayscale=True, confidence=0.8) != None:  # Skip
            pyautogui.press("f3")
            pyautogui.moveTo(950, 285)
            time.sleep(0.3)
            click()
            time.sleep(3)
            pyautogui.typewrite(t6)
            time.sleep(0.3)
            pyautogui.moveTo(kod)
            time.sleep(0.1)
            click()
            time.sleep(0.2)
            pyautogui.press("f3")
